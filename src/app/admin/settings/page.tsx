              ...
              {isLoading ? (
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-center w-full">
                    <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-slate-200 border-dashed rounded-3xl cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors">
                      <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        <Upload className="w-8 h-8 mb-3 text-slate-400" />
                        <p className="mb-2 text-sm text-slate-500 font-bold"><span className="font-semibold text-blue-600">Click to upload</span> or drag and drop</p>
                          ...
                          files.forEach(file => {
                            ...
                            reader.onload = (event) => {
                              ...
                              img.onload = () => {
                                ...
                                ctx?.drawImage(img, 0, 0, width, height);
                                const dataUrl = canvas.toDataURL('image/webp', 0.8);
                                setHomeBanners(prev => [...prev, dataUrl]);
                              };
                              img.src = event.target?.result as string;
                            };
                            reader.readAsDataURL(file);
                  ...
                  {homeBanners.length > 0 && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                      {homeBanners.map((img, idx) => (
                        <div key={idx} className="relative group rounded-2xl overflow-hidden border border-slate-200 aspect-video">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
